...
contract('Remittance', accounts => {
    ...
    it('should deposit correctly', async () => {
        let pw1 = web3.sha3('xyzzy')
        let pw2 = web3.sha3('foobar')
        let password = web3.sha3(pw1.substr(2) + pw2.substr(2), { encoding: 'hex' })
        let blockNumber = await web3.eth.getBlockNumberPromise()
        let depositAmount = web3.toWei(1, 'ether')

        let tx = await remittance.deposit(password, blockNumber + 100, { from: accounts[0], value: depositAmount })
    ...
    it('should prevent multiple deposits under the same password hash', async () => {
        ...
        await remittance.deposit(password, blockNumber + 100, { from: accounts[0], value: web3.toWei(1, 'ether') })
        await expectThrow(
